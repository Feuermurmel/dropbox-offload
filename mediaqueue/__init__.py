...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Minimum number of files to put into the queue. This defaults to '
             'one.')
...
def select_files(size_by_file_by_dir, per_directory_limit, global_limit, size_limit, global_minimum):
    """
    Return a list of tuples (queue, dir, file) for each file where queue is a
    boolean telling whether the file should be queued instead of offloaded,
    dir is the top-level-directory name in which the file is and file is the
